    ...
    public class CertificateController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (!userResultsForFinalCourse.Any())
            {
                ...
                viewModel.Message = "Вы еще не прошли итоговый курс. Пожалуйста, пройдите его, чтобы получить сертификат.";
            }
            else
            {
                // Проверяем, есть ли хотя бы один результат с проходным баллом (80%)
                var hasPassedFinalCourse = userResultsForFinalCourse.Any(r => (double)r.Score / r.TotalQuestions >= 0.8);
                ...
                {
                    viewModel.HasPassedFinalCourse = false;
                    viewModel.Message = "Вы не набрали достаточного балла для получения сертификата по итоговому курсу. Пожалуйста, попробуйте еще раз.";
                }
            }
